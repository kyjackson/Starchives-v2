...
async function updateDb() {
    ...
    // await timer(3000);
    // await updateDbCaptions(key, uploads);

    // update captions for video manually if necessary
    await updateDbCaptionsManual(videoId)
}
...
async function updateDbCaptions(key, playlistId) {
    ...
    //let caption = await executeSQL('select top 1 * from captions')
    //console.log(caption)
...
/**
 * Get a caption track for a specified video upload, reformat it for the database, and update database if necessary
 * 
 * @param    {string}    videoId       ID of the video to retrieve captions for
 */
async function updateDbCaptionsManual(videoId) {

    event.emit("interrupt");
    console.log(`Retrieving caption track for video ${videoId}`);
    ...
    // get captions for the specified videoId, then add to captionObjects array
    ...
    event.emit("interrupt");
    console.log(`Caption track retrieved for video ${videoId}.`);
